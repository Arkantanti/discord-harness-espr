...
export const timerPlugin = {
    ...
    validate(config) {
        ...
        if (typeof config.message !== 'string' || !config.message)
            return 'config.message (string) required';
        ...
        if (!hasDelay && !hasAt)
            return 'config needs delay_ms (number) or at (ISO-8601 string)';
        return null;
    },
    start(ctx) {
        // Persist the absolute fire time so restarts don't lose the reminder.
        let fireAt = ctx.state?.fire_at;
