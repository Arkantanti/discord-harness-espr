...
export async function handleApi(req, res, url) {
    ...
    try {
        ...
        if (parts.length === 2 && parts[1] === 'agents') {
            if (method === 'GET')
                return json(res, 200, agents.listAgents().map(agentSummary)), true;
            if (method === 'POST') {
                const body = await readBody(req);
                if (!body.name || !body.prompt)
                    return json(res, 400, { error: 'name and prompt required' }), true;
        ...
        if (parts.length >= 3 && parts[1] === 'agents') {
            ...
            if (!agent)
                return json(res, 404, { error: 'no such agent' }), true;
            const sub = parts[3];
            if (!sub && method === 'GET') {
            ...
            if (sub === 'message' && method === 'POST') {
                const body = await readBody(req);
                if (!body.content)
                    return json(res, 400, { error: 'content required' }), true;
                if (agent.status === 'terminated')
                    return json(res, 409, { error: 'agent is terminated' }), true;
            ...
            if (sub === 'watchers') {
                if (method === 'GET')
                    return json(res, 200, listWatchers(agent.id)), true;
                if (method === 'POST') {
                    const body = await readBody(req);
                    try {
                ...
                if (wid) {
                    ...
                    if (!w || w.agentId !== agent.id)
                        return json(res, 404, { error: 'no such watcher' }), true;
                    if (method === 'PATCH') {
                        const body = await readBody(req);
                        try {
