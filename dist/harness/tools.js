...
function withLogging(tools) {
    for (const t of tools) {
        const orig = t.handler;
        t.handler = async (args, extra) => {
            try {
 ...
 */
export function buildHarnessServer(callerId) {
    const me = () => agents.getAgent(callerId);
    const tools = [
        tool('spawn_agent', 'Create a new child agent. The prompt becomes its first inbox message (from you). Returns its id and name. Replies from it will arrive in your inbox.', {
        ...
        tool('send_message', 'Send a message to another agent (fire-and-forget). Replies arrive as FUTURE inbox messages — end your turn instead of waiting.', {
            to: z.string().describe('recipient agent id or name'),
            content: z.string(),
        }, async (args) => {
            const target = agents.resolveAgent(args.to);
            if (!target)
                return err(`no agent "${args.to}"`);
            if (target.status === 'terminated')
                return err(`agent "${target.name}" is terminated`);
            messages.enqueue({
                agentId: target.id,
                fromType: 'agent',
                fromId: callerId,
        ...
        tool('get_agent_info', 'Details for one agent: row, watchers, recent activity.', { agent_id: z.string().describe('agent id or name') }, async (args) => {
            const a = agents.resolveAgent(args.agent_id);
            if (!a)
                return err(`no agent "${args.agent_id}"`);
        ...
        tool('terminate_agent', 'Terminate yourself or one of your descendant agents (recursive=true also terminates its children).', {
            agent_id: z.string(),
        ...
        tool('set_watcher', 'Create a watcher for yourself. When it fires, its message lands in YOUR inbox and wakes you. Built-ins: timer {delay_ms|at, message}, cron {schedule, message, timezone?}, poll {command, interval_ms>=5000, mode: on-change|on-output|always, message_prefix?}.', {
            type: z.string(),
        ...
        tool('update_watcher', 'Enable/disable or reconfigure one of your watchers.', {
            watcher_id: z.string(),
            ...
            if (!w || w.agentId !== callerId)
                return err(`no watcher ${args.watcher_id} owned by you`);
            try {
        ...
        tool('remove_watcher', 'Delete one of your watchers.', { watcher_id: z.string() }, async (args) => {
            const w = getWatcher(args.watcher_id);
            if (!w || w.agentId !== callerId)
                return err(`no watcher ${args.watcher_id} owned by you`);
        ...
        tool('reload_watchers', `Rescan ${cfg.watchersDir}/*.mjs and reload all watcher plugins. Call this after writing a new watcher type module.`, {}, async () => {
            const res = await engine.reloadAll();
            return text(`loaded watcher types: ${res.types.join(', ')}` + (res.errors.length ? `\nerrors:\n- ${res.errors.join('\n- ')}` : ''));
        }),
        tool('publish_page', `Publish a web page (HTML or any text). Written under your namespace and served at https://pages.${cfg.domain}/<your-name>/<path> (basic-auth protected).`, {
            path: z.string().describe('relative path, e.g. "index.html" or "reports/week1.html"'),
            content: z.string(),
        }, async (args) => {
            const rel = sanitizePagePath(args.path);
            if (!rel)
                return err('invalid path (must be relative, no "..")');
