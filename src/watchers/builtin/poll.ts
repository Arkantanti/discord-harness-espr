 ...
 */
export const pollPlugin: WatcherPlugin = {
  type: 'poll',
  description: 'Runs a bash command periodically; fires with the output on-change / on-output / always.',
  validate(config: any) {
    if (!config || typeof config !== 'object') return 'config must be an object';
    if (typeof config.command !== 'string' || !config.command) return 'config.command (string) required';
    ...
    if (!['on-change', 'on-output', 'always'].includes(config.mode)) return "config.mode must be 'on-change' | 'on-output' | 'always'";
    return null;
  },
  start(ctx) {
    let busy = false;
    const tick = () => {
      ...
      execFile(
        'bash',
        ['-c', ctx.config.command],
        { timeout: Math.min(ctx.config.interval_ms, 60_000), maxBuffer: 1024 * 1024 },
        (err, stdout, stderr) => {
          busy = false;
          try {
            if (err && err.killed) {
