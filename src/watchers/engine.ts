...
import { loadRegistry, type LoadResult } from './loader.js';
import type { StopFn, WatcherContext, WatcherPlugin, WatcherRow } from '../types.js';
...
class WatcherEngine {
  registry = new Map<string, WatcherPlugin>();
  running = new Map<string, RunningWatcher>();
  ...
  async reloadRegistry(): Promise<LoadResult> {
  ...
  async reloadAll(): Promise<{ types: string[]; errors: string[] }> {
    for (const id of [...this.running.keys()]) await this.stopWatcher(id);
  ...
  validate(type: string, config: unknown): string | null {
    const plugin = this.registry.get(type);
    if (!plugin) return `unknown watcher type "${type}" (known: ${[...this.registry.keys()].join(', ')})`;
    try {
  ...
  async startWatcher(row: WatcherRow, attempt = 0): Promise<void> {
    ...
    const entry: RunningWatcher = { ac, stop: null };
    ...
    const ctx: WatcherContext = {
      ...
      emit: (message, opts) => {
        try {
          ...
          if (!cur || !cur.enabled) return; // stale timer after disable
          messages.enqueue({
            agentId: row.agentId,
            fromType: 'watcher',
            fromId: row.id,
   ...
   */
  scheduleRetry(row: WatcherRow, attempt: number, err: string): void {
    ...
    const timer = setTimeout(() => {
      ...
      const cur = watchers.getWatcher(row.id);
      if (!cur || !cur.enabled) return; // disabled or removed while we waited
  ...
  async addWatcher(opts: { agentId: string; type: string; name?: string | null; config: unknown }): Promise<WatcherRow> {
  ...
  async updateWatcher(id: string, patch: { enabled?: boolean; config?: unknown; name?: string | null }): Promise<WatcherRow> {
    const cur = watchers.getWatcher(id);
    if (!cur) throw new Error(`no watcher ${id}`);
    ...
    if (updated.enabled) await this.startWatcher(updated);
    return updated;
  }
