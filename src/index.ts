...
async function main(): Promise<void> {
  ...
  if (toolPluginErrors.length) console.error('tool plugin errors:', toolPluginErrors);
  await engine.init();
  if (engine.lastLoadErrors.length) console.error('watcher plugin errors:', engine.lastLoadErrors);
  startScheduler();
  startReaper();
  startWebServer();
