...
export function initDb(dbPath: string = cfg.dbPath): void {
...
function migrate(): void {
  const cols = new Set((db.prepare('PRAGMA table_info(agents)').all() as any[]).map((c) => c.name as string));
  // standing = exempt from the idle reaper (long-lived dispatchers etc.)
  if (!cols.has('standing')) db.exec('ALTER TABLE agents ADD COLUMN standing INTEGER NOT NULL DEFAULT 0');
}
