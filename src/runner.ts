...
import type { AgentRow, InboxMessage } from './types.js';
 ...
 */
export async function runTurn(agent: AgentRow, msgs: InboxMessage[], turnId: number, ac: AbortController): Promise<TurnResult> {
  ...
  for await (const m of stream as AsyncGenerator<any>) {
    switch (m.type) {
      case 'assistant': {
  ...
  if (!result) throw new Error('turn ended without a result message');
  return result;
}
